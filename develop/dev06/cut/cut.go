...
func (app *appEnv) run() (string, error) {
	...
	for scanner.Scan() {
		...
		if l == 1 && !app.separated {
			out.WriteString(res[0])
			continue
		}

		if app.toEnd {
			for i := app.fields[0]; i < l; i++ {
				out.WriteString(fmt.Sprintf("%v%v", res[i], app.delimiter))
			}
			continue
		}
		...
		for _, v := range app.fields {
			if v < l {
				out.WriteString(fmt.Sprintf("%v%v", res[v], app.delimiter))
			}
		}
	}
	...
	res := strings.TrimSuffix(out.String(), app.delimiter)
